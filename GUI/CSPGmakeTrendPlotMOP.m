...
for i = 1:length(Icommon)
    ...
    imtimes{i} = datestr(CSPepoch2LocalMatlab(str2num(imdata.epochtime),data.siteDB.timezone.gmt_offset),'mm/dd/yyyy');
...
dates = datenum(imtimes,'mm/dd/yyyy');
if length(Icommon)<sl_cutoff
...
alpha = polyfit(dates(bi),av_bw(bi)',1);
