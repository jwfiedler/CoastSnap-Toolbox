function CSPGmakeShorelineChangePlotWIP(handles)

CSPloadPaths; CSPsetPaths
addpath(fullfile(fileparts(base_path),'MOPS'))

MopNum = 581:589;
transect_nos = 3:8; %%%%
% [t,beachWidth_timeseries] = get_beachwidth(MopNum,base_path);
[~,~,beachSlope_timeseries] = get_beachwidth(MopNum(transect_nos),base_path);
...
% SLtransectsMOP.alongshore_distances = []
...
% pMOP =NaN(2,length(transect_nos)); %beach width matrix to calculate alongshore-average beach width based on defined transects
...
p(1,:) = p(1,:)-bw_corr;
% pMOP(1,:) = pMOP(1,:)-bw_corr;


%Now do present shoreline
imdata = CSPparseFilename(navfiles(Icommon(Inow)).name);
imtimes{2} = datestr(CSPepoch2LocalMatlab(str2num(imdata.epochtime),data.siteDB.timezone.gmt_offset),'mm/dd/yyyy');
sldir = fullfile(shoreline_path,imdata.site,imdata.year);
...
% alongshore_distancesMOP = SLtransects.alongshore_distances(transect_nos);
...
if diffavge<0
    ...
    text(XL(2)+0.1*diff(XL),YL(1)+0.45*diff(YL),[num2str(diffavge,'%0.0f') ' meters (average)'],'fontsize',20,'color','r','fontname','Berlin Sans FB')
else
    plot(XL,diffavge*ones(size(XL)),'g--','linewidth',2 )
    text(XL(2)+0.1*diff(XL),YL(1)+0.7*diff(YL),'Beach width change','fontsize',20,'color','b','fontname','Berlin Sans FB');
    text(XL(2)+0.1*diff(XL),YL(1)+0.45*diff(YL),['+' num2str(diffavge,'%0.0f') ' meters (average)'],'fontsize',20,'color','g','fontname','Berlin Sans FB')
end
